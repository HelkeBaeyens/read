...
def lematization(words):
	...
	def verbs(words): # reverts verbs to their stem
		...
		past_modals = ['did','could','might','would','should']
		clitics_m = ["didn't", "couldn't", "wasn't", "wouldn't", "shouldn't"]
		non_clitics= ['do', 'can', 'was', 'was', 'shall']
		...
		zippy_clitics = dict(zip(clitics_m, non_clitics))
		for word in words:
			if word in irr_SPs:
